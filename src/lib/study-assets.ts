...
export function contentURL(
  ...
  const mapped =
    context.assets && Object.hasOwn(context.assets, value)
  ...
  if (context.statementBase && !value.startsWith("//"))
    return safeWebURL(value, context.statementBase);
  return null;
}

export function solveURL(
  problem: Pick<
    ProblemSummary,
    "platform" | "sourceURL" | "alias" | "externalProblemId"
  >,
): string | null {
  ...
  if (problem.platform === "OMEGAUP")
    return `https://omegaup.com/arena/problem/${encodeURIComponent(identifier)}/`;
  if (problem.platform === "LEETCODE")
    return `https://leetcode.com/problems/${encodeURIComponent(identifier)}/`;
  if (problem.platform === "CSES" && /^\d+$/.test(identifier))
    return `https://cses.fi/problemset/task/${identifier}/`;
  const codeforces = /^(\d+)([A-Za-z]\d*)$/.exec(identifier);
  if (problem.platform === "CODEFORCES" && codeforces)
    return `https://codeforces.com/problemset/problem/${codeforces[1]}/${codeforces[2].toUpperCase()}`;
  return null;
}
...
export function parseProblemConstraints(
  source: string,
  photos: StudyPhoto[] = [],
  apiBase?: string,
  language = "",
  ...
  try {
    ...
    return {
      ...result,
      warning: "No se pudieron interpretar los límites importados.",
    };
  }
  if (!imported)
    return {
      ...result,
      warning: "El formato de los límites no es compatible.",
    };
  ...
  for (const [keys, label, unit] of knownLimits) {
    ...
    if (
      value === undefined ||
      result.limits.some((limit) => limit.label === label)
    )
      continue;
    ...
    if (typeof value === "number" && unit === "bytes")
      formatted =
        value >= 1048576
          ? `${Math.round((value / 1048576) * 100) / 100} MiB`
          : `${value} bytes`;
  ...
  const collectSources = (value: unknown, key = "", depth = 0) => {
    ...
    if (typeof value === "string") {
      ...
      value.forEach((entry) => collectSources(entry, key, depth + 1));
    else {
      ...
      if (typeof data.url === "string")
        collectSources(
          data.url,
          typeof data.language === "string" ? data.language : key,
          depth + 1,
        );
      else
        for (const [name, entry] of Object.entries(data))
          collectSources(entry, name, depth + 1);
    }
  };
  collectSources(imported.statement_sources);
  result.statementBase =
    sources.find((source) => source.language === language)?.url ??
    sources[0]?.url;
  const collectImages = (value: unknown, key = "", depth = 0) => {
    ...
    if (typeof value === "string") {
      ...
      value.forEach((entry) => collectImages(entry, key, depth + 1));
    else {
      const data = object(value);
      if (!data) return;
      const reference = [data.path, data.filename, data.name, key].find(
        (entry) => typeof entry === "string" && entry,
      );
      const target = [data.url, data.src, data.source, data.path].find(
        (entry) => typeof entry === "string" && entry,
      );
      if (typeof reference === "string" && typeof target === "string") {
        ...
        for (const [name, entry] of Object.entries(data))
          collectImages(entry, name, depth + 1);
    }
  };
