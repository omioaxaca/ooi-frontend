...
function text(value: unknown): string {
  ...
  if (typeof value !== "string")
    throw new StudyContentError("Se esperaba contenido de texto.");
  return value;
}
...
function document(value: unknown): RecordValue & { documentId: string } {
  ...
  if (
    ...
    throw new StudyContentError(
      "Se esperaba un documento de Strapi 5 con documentId.",
    );
  }
...
function relation(data: RecordValue, key: string): unknown {
  if (!(key in data)) {
    throw new StudyContentError(
      `No se recibió la relación ${key}. Revisa populate y los permisos de lectura.`,
    );
  }
  return data[key];
}
...
function array(value: unknown): unknown[] {
  if (!Array.isArray(value))
    throw new StudyContentError(
      "Se esperaba una relación o componente de tipo lista.",
    );
  return value;
}
...
export function isMigrationTag(tag: string): boolean {
  return (
    tag === "study-guide-import-v1" ||
    tag.startsWith("study-guide-source:") ||
    tag === "omegaup-syllabus-import-v1" ||
    tag.startsWith("omegaup-syllabus-link-v1:")
  );
}
...
export function normalizeTags(value: unknown): {
  ...
  const values = array(value)
    .map((component) => text(record(component).value))
    .filter(Boolean);
  return {
    ...
    sourcePaths: [
      ...new Set(
        values
          .filter((tag) => tag.startsWith("study-guide-source:"))
...
export function normalizeTopic(value: unknown): TopicSummary {
  ...
  if (!level)
    throw new StudyContentError("El tema tiene un nivel desconocido.");
...
export function uniqueDocuments<Document extends { documentId: string }>(
  values: Document[],
): Document[] {
  return [
    ...new Map(values.map((value) => [value.documentId, value])).values(),
  ];
}
...
export function normalizeProblem(value: unknown): ProblemSummary {
  ...
  return {
    ...
    syllabi: uniqueDocuments(
      array(relation(data, "syllabi")).map(normalizeTopic),
    ),
  };
}
...
export function normalizeProblemDetail(value: unknown): StudyProblem {
  const data = document(value);
  const photos = relation(data, "photos");
  return {
    ...normalizeProblem(data),
    description: text(data.description),
    constraints: text(data.constraints),
    photos: array(photos === null ? [] : photos).map((value) => {
      const photo = record(value);
      return {
        url: text(photo.url),
        name: text(photo.name),
        alternativeText: text(photo.alternativeText),
      };
    }),
  };
}
...
export async function fetchStudyCollection<T extends { documentId: string }>(
  ...
  for (let page = 1; ; page++) {
    const response = record(
      await get(
        `/api/${collection}?${qs.stringify(
          {
            ...query,
            pagination: { page, pageSize: 100, withCount: true },
          },
    ...
    if (
      ...
      throw new StudyContentError(
        "No se pudo verificar la paginación del contenido.",
      );
    }
    ...
    if (entries.length === 0)
      throw new StudyContentError(
        "La paginación devolvió una página incompleta.",
      );
  }
}
...
export function problemHref(
  problem: Pick<ProblemSummary, "documentId">,
  topic?: string,
): string {
...
export function legacyGuideDestination(
  category: string,
  slug: string,
): string | null {
  const source = sourcePath(`${category}/${slug}.mdx`);
  return source
    ? `/dashboard/syllabus/guides/legacy?source=${encodeURIComponent(source)}`
    : null;
}

export function orderRelated<T extends { documentId: string }>(
  values: T[],
  references: unknown,
): T[] {
  const order = array(references).map((value) => document(value).documentId);
  const byId = new Map(
    uniqueDocuments(values).map((value) => [value.documentId, value]),
  );
  const ordered: T[] = [];
...
export function groupStudyTopics(topics: TopicSummary[]) {
  const groups = new Map<
    string,
    { category: StudyCategory | null; topics: TopicSummary[] }
  >();
  for (const topic of topics) {
    const key = topic.category?.documentId ?? "uncategorized";
    if (!groups.has(key))
      groups.set(key, { category: topic.category, topics: [] });
...
export function createStudyRepository(get: StudyGet) {
  const queryURL = (collection: string, query: RecordValue, id?: string) => {
    if (id !== undefined && !/^[a-zA-Z0-9_-]{1,200}$/.test(id))
      throw new StudyContentError("Documento no encontrado.", 404);
  ...
  const curriculum = async (
    ...
    const topics = await fetchStudyCollection(
      get,
      "syllabi",
      {
        fields: topicFields,
    ...
    return topics.sort(
      (first, second) =>
        studyLevels.findIndex((entry) => entry.value === first.level) -
          studyLevels.findIndex((entry) => entry.value === second.level) ||
        first.rank - second.rank ||
        first.documentId.localeCompare(second.documentId),
    );
  };
  const guides = (filters: RecordValue) =>
    fetchStudyCollection(
      ...
      "study-guides",
      {
        ...
        filters,
        sort: ["createdDate:asc", "documentId:asc"],
      },
      normalizeGuide,
    );
  const topicRecord = (id: string, details: boolean) =>
    single("syllabi", id, {
      fields: details
        ? [...topicFields, "body", "externalReferences"]
        : topicFields,
      populate: {
        ...
        ...(details
          ? {
              problems: { fields: ["documentId"] },
              youtubeLinks: tagPopulate,
              pdfLinks: tagPopulate,
            }
          : {}),
      },
    });
  ...
  return {
    ...
    async topic(id: string): Promise<TopicContent> {
      const [data, relatedGuides, relatedProblems] = await Promise.all([
        ...
        fetchStudyCollection(
          ...
          "problems",
          {
            ...
            populate: problemPopulate,
            filters: { syllabi: { documentId: { $eq: id } } },
            sort: ["documentId:asc"],
          },
          normalizeProblem,
        ),
      ...
      return {
        ...normalizeTopic(data),
        body: text(data.body),
        externalReferences: text(data.externalReferences),
        youtubeLinks: array(relation(data, "youtubeLinks"))
          .map((value) => text(record(value).value))
          .filter(Boolean),
        pdfLinks: array(relation(data, "pdfLinks"))
          .map((value) => text(record(value).value))
          .filter(Boolean),
    ...
    async legacyGuide(path: string): Promise<StudyGuide> {
      ...
      const tags = ["mdx", "md"].flatMap((extension) => [
        `study-guide-source:${stem}.${extension}`,
        `study-guide-source:content/docs/${stem}.${extension}`,
      ]);
      const matches = await guides({ tags: { value: { $in: tags } } });
      if (!matches.length)
        throw new StudyContentError(
          "Esta guía todavía no está disponible en el servidor.",
          404,
        );
      if (matches.length !== 1)
        throw new StudyContentError(
          "Hay más de una guía asociada a esta URL. Revisa la importación.",
          409,
        );
    ...
    async directory(
      ...
      if (filter.tag) {
        if (isMigrationTag(filter.tag))
          throw new StudyContentError("Etiqueta no encontrada.", 404);
        return guides({ tags: { value: { $eq: filter.tag } } });
      }
      if (filter.category) {
        if (!/^[a-zA-Z0-9_-]+$/.test(filter.category))
          throw new StudyContentError("Categoría no encontrada.", 404);
        return guides({
          $or: [
            {
              tags: {
                value: {
                  $startsWith: `study-guide-source:${filter.category}/`,
                },
              },
            },
            {
              tags: {
                value: {
                  $startsWith: `study-guide-source:content/docs/${filter.category}/`,
                },
              },
            },
          ],
    ...
    async guideNavigation(current: StudyGuide): Promise<GuideNavigation> {
      ...
      const siblings = current.syllabus
        ? await topicGuides(current.syllabus.documentId)
        : await guides({ syllabus: { documentId: { $null: true } } });
      const index = siblings.findIndex(
        (entry) => entry.documentId === current.documentId,
      );
      if (index === -1)
        throw new StudyContentError(
          "No se pudo verificar la relación de la guía con su tema.",
        );
      ...
      const position = topics.findIndex(
        (topic) => topic.documentId === current.syllabus?.documentId,
      );
      if (current.syllabus && position === -1)
        throw new StudyContentError("No se pudo cargar el tema de la guía.");
      if (current.syllabus) {
        ...
        if (!next)
          next =
            (await guides({ syllabus: { documentId: { $null: true } } }))[0] ??
            null;
      } else if (!previous) {
        for (
          let cursor = topics.length - 1;
          cursor >= 0 && !previous;
          cursor--
        ) {
          previous =
            (await topicGuides(topics[cursor].documentId)).at(-1) ?? null;
        }
      }
    ...
    async problem(id: string): Promise<StudyProblem> {
      const [data, parents] = await Promise.all([
        single("problems", id, {
          ...
          populate: {
            ...problemPopulate,
            photos: { fields: ["url", "name", "alternativeText"] },
          },
        }),
