...
export function studyErrorResponse(error: unknown): Response {
  ...
  const status =
    upstreamStatus &&
    [400, 401, 403, 404, 409, 422, 429, 503].includes(upstreamStatus)
      ? upstreamStatus
      : 502;
  ...
  if (status === 401) message = "Tu sesión expiró. Inicia sesión de nuevo.";
    ...
    message =
      "No tienes permiso para consultar este contenido. Solicita acceso al equipo de OOI.";
  else if (status === 404)
    message = "No se encontró este contenido en el servidor.";
  else if (
    error instanceof StudyContentError ||
    error instanceof RichContentError
  )
...
export async function serveStudyRequest(
  ...
  try {
    ...
    if (!authorization || !/^Bearer [a-zA-Z0-9._~-]+$/.test(authorization))
      throw new StudyContentError("Inicia sesión para estudiar.", 401);
    const repository = createStudyRepository(
      getForRequest(authorization, request.signal),
    );
    ...
    if (segments.length === 1 && resource === "curriculum") {
      ...
      const [content, descriptionContent, referenceContent] = await Promise.all(
        [
          ...
          renderRichContent(topic.externalReferences, {
            format: "markdown",
            apiBase,
          }),
        ],
      ...
      data = {
        ...
        youtubeLinks: topic.youtubeLinks
          .map((url) => safeWebURL(url))
          .filter(Boolean),
        pdfLinks: topic.pdfLinks
          .map((url) => contentURL(url, false, { apiBase }))
          .filter(Boolean),
      };
    } else if (segments.length === 2 && resource === "guides") {
      const guide =
        id === "legacy"
          ? await repository.legacyGuide(parameters.get("source") ?? "")
          : await repository.guide(id);
      const [content, navigation] = await Promise.all([
        renderRichContent(guide.body, { apiBase }),
        repository.guideNavigation(guide),
      ]);
      data = { ...guide, content, navigation };
    } else if (segments.length === 2 && resource === "problems") {
      const problem = await repository.problem(id);
      const constraints = parseProblemConstraints(
        problem.constraints,
        problem.photos,
        apiBase,
        problem.language,
      ...
      const [content, constraintContent] = await Promise.all([
        renderRichContent(problem.description, {
          ...constraints,
          format: "markdown",
        }),
        renderRichContent(constraints.prose, {
          ...constraints,
          format: "markdown",
        }),
      ]);
      data = {
        ...
        constraintNotice: constraints.warning,
