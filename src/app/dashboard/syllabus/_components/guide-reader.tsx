...
export function GuideReader({
  ...
  const request =
    documentId === "legacy"
      ? `guides/legacy?source=${encodeURIComponent(source ?? "")}`
      : `guides/${encodeURIComponent(documentId)}`;
  ...
  useEffect(() => {
    ...
    if (documentId === "legacy")
      router.replace(guideHref(resource.data) + window.location.hash, {
        scroll: false,
      });
    const scrollToHeading = () => {
      try {
        ...
        const heading =
          ...
          document.getElementById(`study-${hash}`);
  ...
  const sidebar = {
    topics: guide.navigation.curriculum,
    currentTopic: guide.syllabus?.documentId,
    guides: guide.navigation.siblings,
    currentGuide: guide.documentId,
  };
  const back = guide.syllabus
    ? topicHref(guide.syllabus)
    : "/dashboard/syllabus/library";
  return (
          ...
          <article className="min-w-0 max-w-3xl space-y-8">
            <header className="space-y-4">
              ...
              >
                ...
                <span>{guide.syllabus?.title ?? "MISC"}</span>
              </Link>
              ...
              <details className="border-y py-3">
                ...
                <nav aria-label="Contenido de la guía" className="mt-3">
                  <ul className="space-y-2 text-sm">
                    {guide.content.headings.map((heading) => (
                      <li
                        key={heading.id}
                        className={heading.depth > 2 ? "ml-4" : ""}
                      >
                        <a
                          href={`#${heading.id}`}
            ...
            <nav
              aria-label="Guía anterior y siguiente"
              className="grid gap-4 border-t pt-6 sm:grid-cols-2"
            >
              <div>
                {guide.navigation.previous && (
                  <Link
                    href={guideHref(guide.navigation.previous)}
                    className="flex h-full items-start gap-2 rounded-md border p-4 text-sm hover:bg-accent"
                  >
                    <ArrowLeft className="h-4 w-4 shrink-0" />
                    <span className="min-w-0 break-words">
                      ...
                      {guide.navigation.previous.title}
                    </span>
                  </Link>
                )}
              </div>
              <div>
                {guide.navigation.next && (
                  <Link
                    href={guideHref(guide.navigation.next)}
                    className="flex h-full items-start justify-end gap-2 rounded-md border p-4 text-right text-sm hover:bg-accent"
                  >
                    <span className="min-w-0 break-words">
                      <span className="mb-1 block text-xs text-muted-foreground">
                        Siguiente
                      </span>
                      {guide.navigation.next.title}
                    </span>
