...
export function TopicResources({ documentId }: { documentId: string }) {
  const resource = useStudyResource<TopicPayload>(
    `topics/${encodeURIComponent(documentId)}`,
  );
  if (resource.loading) return <StudyLoading />;
  if (resource.error)
    return <StudyError error={resource.error} retry={resource.retry} />;
  if (!resource.data) return null;
  return <TopicMaterial topic={resource.data} />;
}
...
export function TopicMaterial({ topic }: { topic: TopicPayload }) {
  return (
    <div className="min-w-0 space-y-8">
      {topic.body.trim() && (
        <section aria-label="Material del tema">
          <MDXContent content={topic.content} />
        </section>
      )}
      <section id="guias" className="scroll-mt-20">
        <h2 className="flex items-center gap-2 text-lg font-semibold text-ooi-dark-blue">
          <BookOpen className="h-5 w-5" />
          Guías de estudio
          <Badge variant="secondary">{topic.guides.length}</Badge>
        </h2>
      ...
      <section id="problemas" className="scroll-mt-20">
        <h2 className="flex items-center gap-2 text-lg font-semibold text-ooi-dark-blue">
          ...
          <Badge variant="secondary">{topic.problems.length}</Badge>
        </h2>
        <ProblemList
      ...
      {topic.youtubeLinks.length > 0 && (
        <section className="space-y-3">
          ...
          {topic.youtubeLinks.map((url) => (
            <a
              key={url}
              href={url}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-start gap-2 text-sm text-ooi-second-blue hover:underline"
            >
      ...
      {topic.externalReferences.trim() && (
        <section className="space-y-3">
          <h2 className="text-lg font-semibold">Referencias</h2>
          <MDXContent content={topic.referenceContent} />
        </section>
      )}
    </div>
  );
...
export function TopicReader({ documentId }: { documentId: string }) {
  ...
  if (resource.error)
    return <StudyError error={resource.error} retry={resource.retry} />;
  if (curriculum.error)
    return <StudyError error={curriculum.error} retry={curriculum.retry} />;
  if (!resource.data || !curriculum.data) return null;
  const topic = resource.data;
  const index = curriculum.data.findIndex(
    (entry) => entry.documentId === topic.documentId,
  );
  ...
  return (
    <div className="min-w-0 space-y-6">
      ...
      <Link
        href={levelHref(topic.level)}
        className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"
      >
        <ArrowLeft className="h-4 w-4" />
        {topic.category?.name || "Temario del nivel"}
      </Link>
        ...
        <div>
          {previous && (
            ...
            >
              ...
              <span>Tema anterior: {previous.title}</span>
            </Link>
          )}
        </div>
        <div>
          {next && (
            <Link
              href={topicHref(next)}
              className="flex items-start justify-end gap-2 text-right text-sm hover:text-primary"
            >
              <span>Siguiente tema: {next.title}</span>
