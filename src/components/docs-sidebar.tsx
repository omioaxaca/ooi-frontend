    ...
    <div className="space-y-3">
      {studyLevels.map((level) => {
        ...
        return (
          <details
            key={level.slug}
            open={entries.some((topic) => topic.documentId === currentTopic)}
            className="group"
          >
            <summary className="cursor-pointer rounded-md px-2 py-2 text-sm font-semibold hover:bg-accent">
            ...
            {groupStudyTopics(entries).map((group) => (
              <div
                ...
                className="mb-4"
              >
                <h3 className="px-2 py-2 text-xs font-semibold text-muted-foreground break-words">
                  ...
                  {group.topics.map((topic) => (
                    <li key={topic.documentId}>
                      <Link
                        ...
                        onClick={onNavigate}
                        className={cn(
                          "block rounded-md px-2 py-2 text-sm break-words hover:bg-accent",
                          topic.documentId === currentTopic &&
...
function GuideLinks({
  ...
  currentGuide,
  onNavigate,
}: Pick<DocsSidebarProps, "guides" | "currentGuide"> & {
  onNavigate?: () => void;
}) {
  return (
    <ul className="ml-2 space-y-1 border-l pl-2">
      {guides.map((guide) => (
        <li key={guide.documentId}>
          <Link
            href={guideHref(guide)}
            onClick={onNavigate}
            aria-current={
              guide.documentId === currentGuide ? "page" : undefined
            }
            className={cn(
              "flex items-start gap-2 rounded-md px-2 py-2 text-sm break-words hover:bg-accent",
              guide.documentId === currentGuide
    ...
    <aside className="hidden w-60 shrink-0 xl:block">
      <div className="sticky top-4 h-[calc(100vh-2rem)]">
        <ScrollArea className="h-full pr-4">
          <nav aria-label="Temario de estudio">
            <Navigation key={pathname} {...props} />
          </nav>
        </ScrollArea>
      </div>
    </aside>
...
export function DocsMobileSidebar(props: DocsSidebarProps) {
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="xl:hidden">
      <CollapsibleTrigger className="mb-4 flex items-center gap-2 rounded-md border px-3 py-2 text-sm font-medium">
      ...
      <CollapsibleContent>
        <nav aria-label="Temario de estudio" className="mb-6 border-y py-3">
          <Navigation
            key={pathname}
            {...props}
