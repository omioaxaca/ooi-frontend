...
test("read-only API requires the student's bearer token and never substitutes an admin token", async () => {
  ...
  const unauthorized = await serveStudyRequest(
    new Request("https://frontend.test/api/study/curriculum"),
    ["curriculum"],
    factory,
  );
  assert.equal(unauthorized.status, 401);
  assert.equal(calls, 0);
  const authorized = await serveStudyRequest(
    new Request("https://frontend.test/api/study/curriculum", {
      headers: { Authorization: "Bearer student-test-session" },
    }),
    ["curriculum"],
    factory,
  );
...
test("permission and missing-document responses are distinct from empty collections and omit private errors", async () => {
  for (const status of [401, 403, 404, 429]) {
    const result = studyErrorResponse({
      response: { status },
  ...
  const result = await serveStudyRequest(
    new Request("https://frontend.test/api/study/users", {
      headers: { Authorization: "Bearer student-test-session" },
    }),
    ["users"],
    () => async () => {
      throw new Error("Must not query arbitrary collections");
    },
  );
...
test("problem details accept null media, exhaust inverse topic pagination, and render useful imported limits/assets", async () => {
  ...
  const requests = [];
  const response = await serveStudyRequest(
    new Request("https://frontend.test/api/study/problems/shared", {
    ...
    () => async (url) => {
      ...
      if (pathname === "/api/problems/shared")
        return {
          data: {
            documentId: "shared",
            title: "Suma",
            ...
            constraints: JSON.stringify({
              ...
              settings: { secret: "private metadata" },
            }),
            externalProblemId: "suma-alias",
            alias: "suma-alias",
            platform: "OMEGAUP",
            difficulty: "easy",
            language: "es",
            qualitySeal: true,
            tags: [{ value: "omegaup-syllabus-import-v1" }, { value: "sumas" }],
            photos: null,
            syllabi: [parent("first")],
          },
        };
      ...
      return {
        data: [parent(page === 1 ? "first" : "second")],
  ...
  assert.deepEqual(
    data.syllabi.map((entry) => entry.documentId),
    ["first", "second"],
  );
  assert.deepEqual(data.tags, ["sumas"]);
  assert.deepEqual(data.limits, [{ label: "Tiempo", value: "1000 ms" }]);
  assert.ok(
    JSON.stringify(data.content.tree).includes(
      "https://api.test/uploads/figure.png",
    ),
  );
  assert.ok(
    !JSON.stringify(data.constraintContent).includes("private metadata"),
  );
...
test("client requests the same-origin API, unwraps data once, and preserves explicit errors instead of returning empty arrays", async () => {
  ...
  axiosInstance.defaults.adapter = async (config) => {
    ...
    if (config.url.endsWith("forbidden"))
      throw new AxiosError("Forbidden", "ERR_BAD_REQUEST", config, null, {
        status: 403,
        data: { error: { message: "Sin permiso" } },
        headers: {},
        config,
    ...
    return {
      status: 200,
      data: config.url.endsWith("invalid") ? {} : { data: [] },
      headers: {},
      config,
      statusText: "OK",
    };
  ...
  try {
    ...
    await assert.rejects(fetchStudyData("forbidden"), {
      status: 403,
      message: "Sin permiso",
    });
