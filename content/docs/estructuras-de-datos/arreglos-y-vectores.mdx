...
**Objetivo:** guardar una secuencia, distinguir sus índices de sus valores y recorrer exactamente los elementos que existen. Necesitas ciclos y tipos de datos. Un vector de tamaño cero es válido, pero no tiene primer ni último elemento.

## Tamaño no es capacidad

`reserve` aparta almacenamiento para posibles inserciones, pero no crea elementos. `resize` sí cambia el tamaño lógico. El límite para acceder con `[]` es `size()`, no `capacity()`.
...
int main() {
    ...
    datos[1] = 7;
    datos.push_back(9);
    cout << datos.size() << ' ' << datos[0] << ' ' << datos[1] << ' ' << datos.back() << '\n';
}
```
...
Acceder por índice cuesta $O(1)$. `push_back` cuesta $O(1)$ amortizado, pero una inserción individual puede realocar y copiar o mover $O(n)$ elementos. Esa realocación puede invalidar referencias, apuntadores e iteradores anteriores. Insertar o borrar en medio normalmente desplaza elementos y cuesta $O(n)$.

**Autocomprobación:** después de `reserve(100)`, ¿puedes escribir `datos[50]` si el tamaño sigue siendo cero? No. Durante depuración, `at(indice)` comprueba límites y lanza una excepción; `[]` no lo hace. Un arreglo `int datos[cantidad]` con tamaño leído en ejecución es una extensión de algunos compiladores: usa `vector<int> datos(cantidad)` en C++17 estándar.
...
<Callout type="warning">
  **¡Los índices empiezan en 0!** Un arreglo de tamaño `n` tiene índices del `0`
  al `n-1`. Acceder a `arr[n]` o `arr[-1]` es un error grave (undefined
  behavior) que puede causar resultados impredecibles o que tu programa se
  cierre.
</Callout>
...
<Callout type="info">
  Los límites de la pila dependen del entorno. Los arreglos globales no ocupan
  esa pila, pero también cuentan para el límite de memoria del juez. Un vector
  permite reservar almacenamiento dinámico según la entrada; ninguna opción
  ofrece memoria ilimitada.
</Callout>
...
[Cadenas: string](/blog/estructuras-de-datos/strings).
