...
`struct` es una construcción del **lenguaje C++**, no un contenedor STL. `pair` y `vector` sí pertenecen a la biblioteca estándar. Necesitas conocer tipos, vectores y funciones para combinar estas herramientas.

## Un registro completo y un par sencillo

Usa campos con nombre cuando su significado importa. Para dos datos estrechamente relacionados, `pair` puede ser suficiente; sus campos se llaman `first` y `second`.
...
int main() {
    vector<Estudiante> grupo = {{"Ana", 10}, {"Luis", 8}};
    grupo[1].puntos += 3;
    pair<int, int> coordenada = {2, 5};
    cout << grupo[1].nombre << ' ' << grupo[1].puntos << '\n';
...
Modificar el puntaje no cambia el nombre del registro. Al ordenar el vector, se mueve el registro completo. Un `pair` se compara lexicográficamente; un `struct` no adquiere automáticamente un orden útil en C++17. Define un comparador cuando llegues a ordenamiento personalizado.

**Autocomprobación:** `for (Estudiante estudiante : grupo)` recorre copias; cambiar sus puntos no modifica el vector. Usa `Estudiante&` para modificar y `const Estudiante&` para consultar sin copiar. Inicializa los campos numéricos para no leer valores indeterminados.
...
<Callout type="info">
  `pair` es muy cómodo, pero para más de dos campos, usa un struct con nombres
  descriptivos. Es más legible `evento.inicio` que `evento.first`.
</Callout>
...
[Pilas y colas: STL stack y queue](/blog/pilas-y-colas/pilas-concepto).
