---
title: "Union-Find (DSU)"
description: "Estructura eficiente para manejar conjuntos disjuntos y consultas de conectividad"
date: "2026-02-09"
author: "OOI Oaxaca"
tags: ["c++", "Union-Find", "DSU", "conjuntos disjuntos"]
syllabusId: 60
---

## ¿Qué es Union-Find?

**Objetivo:** mantener una partición en componentes mientras se agregan conexiones. `find` devuelve un representante interno, no necesariamente el menor vértice ni un vecino real del grafo. Solo importa si dos representantes coinciden.

**Analogía:** Imagina un grupo de personas. Al principio, cada persona está sola. Cuando dos personas se hacen amigas, sus grupos se fusionan (si Ana es amiga de Luis, y Luis de Pedro, entonces Ana y Pedro están en el mismo grupo). Union-Find maneja esto eficientemente.

Soporta dos operaciones:

- **Find(x):** ¿A qué grupo pertenece x?
- **Union(a, b):** Fusionar los grupos de a y b.

## Implementación con optimizaciones

```cpp test
#include <algorithm>
#include <iostream>
using namespace std;

const int MAXN = 100005;
int padre[MAXN], rango[MAXN], tam[MAXN];

void inicializar(int n) {
    for (int i = 1; i <= n; i++) {
        padre[i] = i;
        rango[i] = 0;
        tam[i] = 1;
    }
}

// Find con compresión de caminos
int encontrar(int x) {
    if (padre[x] != x) {
        padre[x] = encontrar(padre[x]);
    }
    return padre[x];
}

// Union por rango
bool unir(int a, int b) {
    a = encontrar(a);
    b = encontrar(b);
    if (a == b) return false;

    if (rango[a] < rango[b]) swap(a, b);
    padre[b] = a;
    tam[a] += tam[b];
    if (rango[a] == rango[b]) rango[a]++;

    return true;
}

bool conectados(int a, int b) {
    return encontrar(a) == encontrar(b);
}

int tamComponente(int x) {
    return tam[encontrar(x)];
}

int main() {
    inicializar(4);
    cout << unir(1, 2) << ' ' << unir(2, 3) << ' ' << unir(1, 3) << '\n';
    cout << tamComponente(1) << ' ' << conectados(1, 4) << '\n';
}
```

Salida:

```text output
1 1 0
3 0
```

La tercera unión no cambia la partición. Solo debes disminuir el contador de componentes cuando `unir` devuelve verdadero. Los tamaños se consultan en la raíz, no en el índice original del elemento.

DSU no conserva rutas del grafo ni permite eliminar aristas de forma general. Para eliminaciones offline existen estrategias distintas, como rollback; para consultar versiones históricas necesitarás persistencia. No apliques esta conectividad no dirigida para decidir alcanzabilidad dirigida.

**Autocomprobación:** unir un elemento consigo mismo no cambia nada. Después de unir 1 con 2 y 2 con 3, 1 y 3 están conectados aunque no exista una arista directa entre ellos.

### Compresión de caminos

Sin optimización, `encontrar` puede ser $O(n)$. Con compresión, cada nodo apunta directamente a la raíz:

```
Antes:  1 → 2 → 3 → 4 (raíz)
Después: 1 → 4, 2 → 4, 3 → 4
```

### Unión por rango

Antes de la compresión, el rango describe una cota de altura. Después puede no coincidir con la altura real, pero sigue siendo útil para decidir qué raíz colgar. Unión por rango y compresión juntas dan la cota amortizada siguiente; no se trata de que cada llamada aislada sea literalmente constante.

**Complejidad amortizada:** $O(\alpha(n))$ ≈ $O(1)$ por operación.

## Aplicaciones

### Contar componentes después de agregar aristas

```cpp
int main() {
    int n, m;
    cin >> n >> m;
    inicializar(n);

    int componentes = n;
    for (int i = 0; i < m; i++) {
        int u, v;
        cin >> u >> v;
        if (unir(u, v)) {
            componentes--;
        }
        cout << "Componentes: " << componentes << endl;
    }
}
```

### Detectar ciclos al agregar aristas

```cpp
for (auto [u, v] : aristas) {
    if (!unir(u, v)) {
        cout << "Arista (" << u << "," << v << ") forma un ciclo" << endl;
    }
}
```

### Kruskal (MST)

Union-Find es fundamental para el algoritmo de Kruskal (ver sección de MST).

## Ejercicio de práctica

N personas y Q operaciones. Operación 1: "A y B son amigos". Operación 2: "¿A y B son amigos (directa o indirectamente)?".

<details>
<summary>Ver solución</summary>

```cpp
#include <iostream>
using namespace std;

int padre[100005], rk[100005];

int encontrar(int x) {
    if (padre[x] != x) padre[x] = encontrar(padre[x]);
    return padre[x];
}

void unir(int a, int b) {
    a = encontrar(a); b = encontrar(b);
    if (a == b) return;
    if (rk[a] < rk[b]) swap(a, b);
    padre[b] = a;
    if (rk[a] == rk[b]) rk[a]++;
}

int main() {
    int n, q;
    cin >> n >> q;
    for (int i = 1; i <= n; i++) { padre[i] = i; rk[i] = 0; }

    while (q--) {
        int op, a, b;
        cin >> op >> a >> b;
        if (op == 1) unir(a, b);
        else cout << (encontrar(a) == encontrar(b) ? "SI" : "NO") << endl;
    }
    return 0;
}
```

</details>
