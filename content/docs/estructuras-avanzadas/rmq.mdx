---
title: "Range Minimum Query (RMQ)"
description: "Preprocesa mínimos estáticos con bloques de potencias de dos y comprende cómo RMQ puede resolver LCA."
date: "2026-02-09"
author: "OOI Oaxaca"
tags: ["c++", "RMQ", "sparse table", "consultas de rango"]
syllabusId: 68
---

## ¿Qué es RMQ?

**Analogía:** Imagina que tienes una fila de montañas y un turista te pregunta: "¿Cuál es la montaña más baja entre la posición 3 y la posición 8?". Si te hacen esta pregunta miles de veces con diferentes rangos, necesitas una forma rápida de responder. Eso es **Range Minimum Query** (consulta del mínimo en un rango).

Dado un arreglo, responder consultas: ¿cuál es el **mínimo** en el rango `[l, r]`?

## Solución ingenua: $O(n)$ por consulta

```cpp
int queryMin(vector<int> &v, int l, int r) {
    int mn = v[l];
    for (int i = l + 1; i <= r; i++) {
        mn = min(mn, v[i]);
    }
    return mn;
}
```

Con Q consultas → $O(n \cdot Q)$. Demasiado lento.

## Sparse Table: $O(n \log n)$ preproceso, $O(1)$ por consulta

**Idea:** Precalcular el mínimo de todos los bloques de tamaño potencia de 2.

`sparse[k][i]` = mínimo del rango que empieza en `i` y tiene longitud $2^k$.

```
sparse[0][i] = v[i]             (bloques de tamaño 1)
sparse[1][i] = min(v[i], v[i+1])  (bloques de tamaño 2)
sparse[2][i] = min de v[i..i+3]   (bloques de tamaño 4)
...
```

### Construcción

```cpp
const int MAXN = 200005;
const int LOG = 18;

int sparse[LOG][MAXN];
int n;
int logaritmo[MAXN];

void build(vector<int> &v) {
    n = v.size();
    logaritmo[1] = 0;
    for (int longitud = 2; longitud <= n; longitud++) logaritmo[longitud] = logaritmo[longitud / 2] + 1;

    // Bloques de tamaño 1
    for (int i = 0; i < n; i++) {
        sparse[0][i] = v[i];
    }

    // Bloques de tamaño 2^k
    for (int k = 1; k < LOG; k++) {
        for (int i = 0; i + (1 << k) - 1 < n; i++) {
            sparse[k][i] = min(
                sparse[k-1][i],                  // Primera mitad
                sparse[k-1][i + (1 << (k-1))]    // Segunda mitad
            );
        }
    }
}
```

**¿Por qué funciona?** Un bloque de tamaño $2^k$ se divide en dos bloques de tamaño $2^{k-1}$:

```
[-------- 2^k --------]
[--- 2^(k-1) ---][--- 2^(k-1) ---]
```

### Consulta en $O(1)$

Para un rango `[l, r]`, encontramos la mayor potencia de 2 que cabe, y usamos dos bloques que cubran todo el rango (pueden traslaparse, ¡no importa para el mínimo!):

```cpp
int query(int l, int r) {
    int len = r - l + 1;
    int k = logaritmo[len];

    return min(
        sparse[k][l],                  // Bloque desde l
        sparse[k][r - (1 << k) + 1]   // Bloque hasta r
    );
}
```

```
Rango [2, 7], longitud 6, k = 2:
[2 3 4 5 6 7]
[2 3 4 5]       bloque izquierdo
    [4 5 6 7]   bloque derecho
Las posiciones 4 y 5 se incluyen dos veces, sin alterar el minimo.
```

<Callout type="info">
  El truco de traslape funciona para **min**, **max** y **GCD**, pero **NO**
  para suma (contarías elementos dos veces).
</Callout>

### Código completo

La tabla `logaritmo` calcula el piso del logaritmo en enteros y permite consultas constantes sin extensiones como `__lg`. Esta versión admite `1 <= n <= 200000` y consultas cerradas con posiciones desde 1. Un intervalo vacío no tiene mínimo y no está admitido.

```cpp test
#include <algorithm>
#include <iostream>
#include <vector>
using namespace std;

const int MAXN = 200005;
const int LOG = 18;
int sparse[LOG][MAXN];
int logaritmo[MAXN];

void build(vector<int> &v, int n) {
    logaritmo[1] = 0;
    for (int longitud = 2; longitud <= n; longitud++) logaritmo[longitud] = logaritmo[longitud / 2] + 1;
    for (int i = 0; i < n; i++)
        sparse[0][i] = v[i];

    for (int k = 1; k < LOG; k++)
        for (int i = 0; i + (1 << k) - 1 < n; i++)
            sparse[k][i] = min(sparse[k-1][i],
                               sparse[k-1][i + (1 << (k-1))]);
}

int query(int l, int r) {
    int k = logaritmo[r - l + 1];
    return min(sparse[k][l], sparse[k][r - (1 << k) + 1]);
}

int main() {
    int n, q;
    cin >> n >> q;

    vector<int> v(n);
    for (int &x : v) cin >> x;

    build(v, n);

    while (q--) {
        int l, r;
        cin >> l >> r;
        l--; r--;  // 0-indexed
        cout << query(l, r) << "\n";
    }

    return 0;
}
```

Entrada:

```text input
5 3
2 1 4 3 5
1 3
2 5
4 4
```

Salida:

```text output
1
1
3
```

## Relación con LCA

En un árbol enraizado, registra cada vértice al entrar y de nuevo al regresar de cada hijo: es un recorrido Euler de longitud $2n-1$. Entre las primeras apariciones de dos vértices, el de menor **profundidad** es su ancestro común más bajo. Una sparse table puede guardar el vértice de profundidad mínima, no el identificador numéricamente menor.

Esto convierte LCA en un RMQ estático. Primero dominarás padres y binary lifting en la sección de árboles; la segunda guía de LCA en Verde desarrolla la reducción completa. Cambiar aristas del árbol invalidaría el recorrido y sus tablas.

## ¿Cuándo usar Sparse Table vs Segment Tree?

| Característica | Sparse Table  | Segment Tree   |
| -------------- | ------------- | -------------- |
| Construcción   | $O(n \log n)$ | $O(n)$         |
| Consulta       | $O(1)$        | $O(\log n)$    |
| Actualización  | ❌ No soporta | ✅ $O(\log n)$ |
| Memoria        | $O(n \log n)$ | $O(4n)$        |

**Regla:** Si el arreglo **no cambia**, usa Sparse Table. Si hay **actualizaciones**, usa Segment Tree.

## Ejercicio

Dado un arreglo de N enteros y Q consultas `(l, r)`, imprime el **máximo** de cada rango.

<details>
<summary>Ver solución</summary>

```cpp test
#include <algorithm>
#include <iostream>
#include <vector>
using namespace std;

const int MAXN = 200005;
const int LOG = 18;
int sparse[LOG][MAXN];
int logaritmo[MAXN];

void build(vector<int> &v, int n) {
    logaritmo[1] = 0;
    for (int longitud = 2; longitud <= n; longitud++) logaritmo[longitud] = logaritmo[longitud / 2] + 1;
    for (int i = 0; i < n; i++)
        sparse[0][i] = v[i];

    for (int k = 1; k < LOG; k++)
        for (int i = 0; i + (1 << k) - 1 < n; i++)
            sparse[k][i] = max(sparse[k-1][i],
                               sparse[k-1][i + (1 << (k-1))]);
}

int query(int l, int r) {
    int k = logaritmo[r - l + 1];
    return max(sparse[k][l], sparse[k][r - (1 << k) + 1]);
}

int main() {
    int n, q;
    cin >> n >> q;
    vector<int> v(n);
    for (int &x : v) cin >> x;
    build(v, n);

    while (q--) {
        int l, r;
        cin >> l >> r;
        l--; r--;
        cout << query(l, r) << "\n";
    }
    return 0;
}
```

Entrada:

```text input
5 2
2 1 4 3 5
1 3
2 5
```

Salida:

```text output
4
5
```

Simplemente cambia `min` por `max` en build y query. El traslape es válido porque repetir un valor no cambia ni el mínimo ni el máximo.

</details>
