---
title: "Funciones: valor, referencia y apuntadores"
description: "Organiza una tarea en funciones y decide cuándo copiar un dato, modificarlo por referencia o usar su dirección."
date: "2026-02-09"
author: "OOI Oaxaca"
...
<Callout type="warning">
  Para consultar un vector o string sin copiarlo, usa una referencia constante.
  Para modificar el original, usa una referencia no constante. Pasar por valor
  también puede ser intencional si necesitas una copia independiente; considera
  su coste.
</Callout>

## Paso mediante apuntadores

Un apuntador guarda una dirección. `&variable` obtiene la dirección de una variable y `*apuntador` permite acceder al objeto situado allí. El parámetro apuntador se copia por valor, pero la dirección copiada sigue señalando el objeto original.

Una referencia debe referirse a un objeto válido al inicializarse. Un apuntador puede valer `nullptr` para representar «ningún objeto»; debes comprobarlo antes de desreferenciarlo. Para un parámetro obligatorio y sencillo, una referencia suele expresar mejor la intención.
...
void porApuntador(int* direccion) {
        if (direccion != nullptr) *direccion = *direccion + 1;
}
...
int main() {
        ...
        porValor(puntos);
        cout << puntos << '\n';
        porReferencia(puntos);
        cout << puntos << '\n';
        porApuntador(&puntos);
        cout << puntos << '\n';
        porApuntador(nullptr);
}
```
...
Los tres llamados no son intercambiables: el primero modifica una copia; los otros dos pueden modificar `puntos`. Cambiar solamente la dirección guardada en el parámetro apuntador no cambia el apuntador del llamador. Tampoco debes devolver la dirección de una variable local que dejará de existir al salir de la función.

**Comprueba tu comprensión:** si la función debe leer un vector grande sin cambiarlo, ¿qué parámetro usarías? `const vector<int>& valores`: permite leerlo, evita copiarlo y prohíbe modificar sus elementos a través de esa referencia.
...
<Callout type="info">
  Las variables globales se inicializan a cero y no ocupan la pila de llamadas.
  Aun así, el almacenamiento estático y la memoria dinámica también cuentan para
  el límite de memoria del programa. Un `vector` permite ajustar el tamaño a la
  entrada.
</Callout>
...
[Ciclos: for, while y do while](/blog/estructuras-de-control/ciclo-for).
