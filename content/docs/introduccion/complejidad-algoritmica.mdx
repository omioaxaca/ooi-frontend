...
**Antes de empezar:** necesitas ciclos, arreglos y operaciones básicas. La meta no es medir segundos con exactitud, sino estimar cómo crecen trabajo y memoria para decidir si una solución cabe en las restricciones.

## Qué afirma Big O y qué no

Big O expresa una cota superior de crecimiento, ignorando constantes y tamaños iniciales pequeños. Decir $O(n)$ no garantiza que al duplicar una entrada real el reloj marque exactamente el doble. Cuando expresamos un crecimiento ajustado como proporcional a $n$, la notación formal es $\Theta(n)$; aquí usaremos cotas Big O útiles para comparar algoritmos.

Identifica todas las variables del problema: recorrer una matriz de $filas$ por $columnas$ cuesta $O(filas\cdot columnas)$, no siempre $O(n^2)$. Leer $n$ datos ya requiere tiempo proporcional a $n$ aunque el cálculo final sea una fórmula.

Dos ciclos anidados no implican siempre $n^2$ pasos. Si el interno empieza donde terminó anteriormente y cada índice solo avanza, ambos pueden sumar $O(n)$ movimientos. En cambio, si para cada posición revisas todas las anteriores, ejecutas $0+1+\cdots+(n-1)=n(n-1)/2$ iteraciones: sigue siendo cuadrático.

**Autocomprobación:** para $n=100000$, una tabla de $n^2$ enteros de 4 bytes necesita alrededor de 40 GB. Un algoritmo puede fallar por memoria antes que por tiempo. Cuenta también tablas auxiliares, copias de vectores y la profundidad de la pila recursiva.
...
- Un trabajo proporcional a $n$ aproximadamente se duplica al duplicar $n$ y tiene cota $O(n)$.
- Un trabajo proporcional a $n^2$ aproximadamente se cuadruplica al duplicar $n$ y tiene cota $O(n^2)$.
...
<Callout type="info">
  En competencias, una computadora puede hacer aproximadamente **10⁸ operaciones
  por segundo** (cien millones). Si el límite de tiempo es 1 segundo, tu
  programa no puede hacer más de ~100,000,000 de operaciones.
</Callout>
...
<Callout type="warning">
  Siempre lee los **límites del problema** (las restricciones o constraints)
  antes de empezar a programar. Si el problema dice que n puede ser hasta 10⁶ y
  tu solución es O(n²), **no funcionará** (10¹² operaciones tardarían ~10,000
  segundos). Necesitas pensar en algo más eficiente.
</Callout>
...
[Ordenamiento: burbuja y STL sort](/blog/algoritmos/ordenamiento).
