...
Esta guía se refiere a grafos **no dirigidos**. En dirigidos, ignorar direcciones produce componentes débiles; exigir caminos en ambos sentidos produce SCC y requiere otro análisis. No confundas esas definiciones.

**Objetivo:** asignar exactamente una etiqueta de componente a cada vértice. Un vértice aislado forma su propia componente, y el grafo vacío tiene cero. Los números de las etiquetas son arbitrarios: compara si coinciden, no su valor absoluto.
...
Cada DFS nuevo comienza en un vértice todavía sin visitar y alcanza exactamente su componente. Conservamos visitados entre esos recorridos, por lo que cada vértice y cada arista se procesan una sola vez: tiempo $O(V+E)$ y memoria $O(V+E)$ incluyendo el grafo. La DFS recursiva requiere profundidad moderada o una conversión iterativa para casos grandes.

**Autocomprobación:** la suma de los tamaños de componentes debe ser el total de vértices. Agregar una arista entre componentes distintas reduce su cantidad en uno; agregarla dentro de una componente no cambia el conteo.
