...
**Objetivo:** mantener el extremo prioritario sin ordenar toda la colección. Un heap binario llena todos los niveles salvo quizá el último, que se llena de izquierda a derecha. Permite padres iguales a sus hijos; la propiedad no exige comparaciones estrictas.
...
Los métodos `maximo` y `eliminarMaximo` exigen que el heap no esté vacío. Comprueba esa condición con `vacio` antes de llamarlos.
...
int main() {
    MaxHeap pendientes;
    for (int valor : {10, 30, 20}) pendientes.insertar(valor);
    while (!pendientes.vacio()) {
        cout << pendientes.maximo() << ' ';
        pendientes.eliminarMaximo();
    }
    cout << '\n';
}
```
...
Solo las relaciones padre-hijo están ordenadas. Dos hermanos pueden aparecer en cualquier orden relativo. Después de insertar, únicamente el camino del nuevo nodo a la raíz puede violar la propiedad; después de eliminar, solo puede violarse en el camino que baja desde la raíz. Ambos tienen longitud $O(\log n)$.
...
Construir un heap bajando desde los últimos padres cuesta $O(n)$: la mayoría de nodos está cerca de las hojas y baja muy poco. Construirlo con $n$ inserciones independientes cuesta $O(n\log n)$. Extraer todos los máximos cuesta $O(n\log n)$ en ambos casos.
...
**Autocomprobación:** prueba valores iguales y un único elemento. Tras extraer el único elemento no debes acceder a la raíz. El heap de esta guía es una estructura de datos; no es el área de memoria dinámica también llamada heap.
