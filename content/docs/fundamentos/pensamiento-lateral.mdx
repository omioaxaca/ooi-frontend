...
**Antes de empezar:** esta primera lección trata de ideas, no de sintaxis. Resuelve los ejemplos en papel; los fragmentos de C++ son una vista previa que podrás retomar después de estudiar ciclos y contenedores. La meta es reformular una pregunta sin cambiar lo que pide.
...
Los ejemplos sugieren una fórmula, pero todavía falta demostrarla. Emparejar el primero con el último produce siempre `n + 1`; al sumar la lista normal y la invertida aparecen `n` de esas parejas. Así obtenemos `2 * suma = n * (n + 1)`. La siguiente lección distingue con más detalle conjetura, contraejemplo y demostración.

<Callout type="info">
  En competencias, una estrategia muy útil es probar tu solución con casos
  pequeños (n=1, 2, 3, 4...) para descubrir patrones. Muchas veces, un problema
  que parece complicado tiene una fórmula simple escondida.
</Callout>
...
Cada búsqueda e inserción en `set` cuesta $O(\log n)$, así que el tiempo total es $O(n\log n)$ y el espacio $O(n)$. Preguntar **antes** de insertar evita usar dos veces la misma posición. Prueba `[5]` con objetivo 10: no hay pareja; con `[5,5]`, sí. Los cálculos de suma y complemento deben caber en el tipo elegido.
...
<Callout type="success">
  El pensamiento lateral es un músculo que se entrena con práctica. Cada
  problema que resuelvas te hará un poco mejor. No te frustres si no ves la
  solución de inmediato; incluso los expertos necesitan tiempo para pensar.
</Callout>

## Siguiente paso

[Razonamiento inductivo y deductivo](/blog/fundamentos/razonamiento-inductivo-deductivo).
