...
**Objetivo de estudio:** poder escribir el contrato de un problema, proponer pasos finitos y explicar por qué producen la salida correcta. Todavía puedes trabajar en lenguaje natural; los bloques C++ muestran cómo se traducirán esas ideas cuando aprendas la sintaxis.
...
<Callout type="warning">
  Si un conjunto de instrucciones no termina nunca (por ejemplo, "cuenta todos
  los números naturales"), **no es un algoritmo**. Los algoritmos SIEMPRE deben
  terminar.
</Callout>
...
**Problema:** dada una lista **no vacía** de números, encontrar el mayor. La condición no vacía importa: el método necesita un primer valor y una lista vacía no tiene máximo sin una convención adicional.
...
Después de cada caja, el valor recordado es el mayor de todas las cajas abiertas. Al abrir otra, comparar y conservar el mayor mantiene esa propiedad. Como quedan menos cajas pendientes, el proceso termina y la propiedad final cubre toda la lista.

**Comprueba:** para `[-8,-3,-5]` debe devolver -3. Empezar con máximo igual a cero daría una respuesta que ni siquiera aparece en la lista. Para una sola caja, devuelve su valor sin comparaciones adicionales. Un algoritmo correcto necesita tanto un contrato claro como casos que intenten refutarlo.
...
<Callout type="info">
  Probar con **casos extremos** (un solo elemento, todos iguales, números
  negativos) es fundamental. Muchos errores se esconden en estos casos.
</Callout>
...
[Configuración de VS Code y computadora](/blog/introduccion/configuracion-entorno).
