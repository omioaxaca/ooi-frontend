...
**Objetivo:** recorrer todos los estados alcanzables sin entrar en un ciclo infinito. Necesitas recursión y una marca de visitado. Marca el estado **antes** de explorar vecinos; de lo contrario una arista de regreso puede volver a invocar el estado todavía pendiente.
...
| Memoria adicional   | Hasta $O(V)$ para visitados y cola | Hasta $O(V)$ para visitados y pila recursiva |
...
El grafo contiene un ciclo, pero cada vértice se procesa una sola vez. El orden depende del orden de las listas de vecinos: no es una respuesta única del grafo. Un recorrido cuesta $O(V+E)$ tiempo contando inicialización y listas; una cadena puede requerir $O(V)$ llamadas pendientes.

DFS no garantiza que el primer camino hallado sea el más corto. Si necesitas momentos de entrada y salida, una pila iterativa debe guardar también qué vecinos quedan por explorar; cambiar simplemente `queue` por `stack` no reproduce automáticamente todos los eventos de una DFS recursiva.

**Practica:** invierte los vecinos de 0 en el ejemplo y traza el nuevo recorrido. Cambia el orden, pero no el conjunto alcanzable. Reinicia visitados entre búsquedas independientes; para componentes, en cambio, conserva las marcas al pasar al siguiente origen.
...
<Callout type="info">
  La diferencia entre BFS y DFS iterativo es solo la estructura: BFS usa
  **cola** (queue), DFS usa **pila** (stack). El resto del código es casi
  idéntico.
</Callout>
...
[Búsqueda en amplitud: BFS](/blog/busquedas/busqueda-amplitud).
