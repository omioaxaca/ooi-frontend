...
**Objetivo:** explicar qué resuelve una llamada, cuándo responde directamente y qué cantidad disminuye hasta llegar a ese caso. Saber que una función se llama a sí misma no basta para garantizar que termine.
...
El ejemplo calcula factorial exacto únicamente para $0\leq n\leq20$, de modo que cabe en `long long`. Un argumento negativo no se acercaría al caso base al restarle uno.
...
int main() {
    int numero;
    cin >> numero;
    if (numero < 0 || numero > 20) return 0;
...
<Callout type="warning">
  La pila tiene un límite que depende del entorno y de cuánto guarda cada
  llamada. Una recursión profunda puede desbordarla. La memorización reduce
  estados repetidos, pero no elimina necesariamente la profundidad: para
  evitarla usa una versión iterativa o una pila explícita cuando corresponda.
</Callout>

Factorial hace $n+1$ llamadas y mantiene hasta $n+1$ pendientes: tiempo y espacio adicional $O(n)$. **Comprueba:** `factorial(0)` debe devolver 1, no 0. Un caso base correcto también debe devolver el elemento neutro que necesita la operación al regresar.
...
<Callout type="warning">
  ¡Esta implementación es **muy lenta**! `fibonacci(40)` tarda varios segundos
  porque recalcula los mismos valores muchas veces. La solución es la
  **memorización**, que veremos en la siguiente lección.
</Callout>
...
[Árbol de llamadas, diseño de estado y recursión sobre arreglos](/blog/recursion/visualizacion-estado).
