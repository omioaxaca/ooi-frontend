---
title: "Introducción a Programación Dinámica"
description: "Aprende la técnica más poderosa de competencias: descomponer problemas en subproblemas"
date: "2026-02-09"
author: "OOI Oaxaca"
tags: ["c++", "programación dinámica", "DP", "memoización", "tabulación"]
syllabusId: 66
---

## ¿Qué es programación dinámica?

**Antes de empezar:** necesitas recursión, memorización y complejidad. En DP diseñamos subproblemas cuya respuesta completa puede reutilizarse. La pregunta esencial es qué información necesita el futuro, no qué tamaño de tabla parece habitual.

**Analogía:** Imagina que te preguntan: "¿Cuánto es 3+5+2+7+1+4?". Calculas: 22. Luego te preguntan: "¿Y si le sumas 6?". No vuelves a sumar todo — usas el resultado anterior: 22 + 6 = 28. ¡Eso es programación dinámica! **Guardar resultados para no recalcularlos.**

Dos ideas centrales son:

1. **Subproblemas superpuestos:** varias decisiones necesitan las respuestas de los mismos estados, por lo que compartirlas evita repetir trabajo.
2. **Subestructura óptima en problemas de optimización:** una decisión puede completarse usando una solución óptima del subproblema restante. DP también sirve para contar y decidir existencia, sin buscar un máximo o mínimo.

Si dos situaciones guardadas en la misma celda tienen futuros diferentes, falta información en el estado. Por ejemplo, «dinero restante» no basta para una mochila con objetos utilizables una vez: también importa cuáles objetos quedan disponibles. En monedas ilimitadas, en cambio, el monto restante sí puede bastar.

Una demostración de optimalidad separa por la última decisión y comprueba que las alternativas cubren todas las soluciones. Sustituir un subproblema por una solución mejor no debe violar restricciones externas: esa es la razón para diseñar bien el estado.

Los ejemplos exactos de Fibonacci con `long long` requieren índices no mayores que 92. Memorización y tabulación evalúan la misma recurrencia, pero la primera puede conservar una pila de profundidad lineal.

## Ejemplo clásico: Fibonacci

Sin DP ($O(2^n)$, ¡terrible!):

```cpp
int fib(int n) {
    if (n <= 1) return n;
    return fib(n-1) + fib(n-2);  // Recalcula todo
}
```

### Top-Down (Memoización)

Recursión + guardar resultados:

```cpp
long long memo[105];
bool calculado[105];

long long fib(int n) {
    if (n <= 1) return n;
    if (calculado[n]) return memo[n];

    calculado[n] = true;
    memo[n] = fib(n-1) + fib(n-2);
    return memo[n];
}
```

### Bottom-Up (Tabulación)

Llenar una tabla iterativamente:

```cpp
long long fib(int n) {
    long long dp[105];
    dp[0] = 0;
    dp[1] = 1;

    for (int i = 2; i <= n; i++) {
        dp[i] = dp[i-1] + dp[i-2];
    }

    return dp[n];
}
```

Ambos son $O(n)$.

## Los 4 pasos para resolver con DP

<Steps>
  <Step>
    **Definir el estado.** ¿Qué representa `dp[i]`? (o `dp[i][j]`, etc.)
  </Step>
  <Step>
    **Encontrar la transición.** ¿Cómo se calcula `dp[i]` a partir de estados
    anteriores?
  </Step>
  <Step>**Establecer casos base.** ¿Cuáles son los valores iniciales?</Step>
  <Step>**Determinar la respuesta.** ¿Dónde está la respuesta final?</Step>
</Steps>

## Problema clásico: Monedas

Dado un conjunto de monedas, ¿cuál es el **mínimo número de monedas** para dar cambio de una cantidad C?

```
Monedas: {1, 5, 10, 25}
Cantidad: 36
Respuesta: 3 (25 + 10 + 1)
```

**Paso 1 — Estado:** `dp[c]` = mínimo de monedas para la cantidad `c`.

**Paso 2 — Transición:** Para cada moneda `m`, si `c >= m`, entonces `dp[c] = min(dp[c], dp[c - m] + 1)`.

**Paso 3 — Caso base:** `dp[0] = 0` (0 monedas para dar cambio de 0).

**Paso 4 — Respuesta:** `dp[C]`.

```cpp test
#include <algorithm>
#include <iostream>
#include <vector>
#include <climits>
using namespace std;

int main() {
    int n, C;
    cin >> n >> C;

    vector<int> monedas(n);
    for (int &m : monedas) cin >> m;

    vector<int> dp(C + 1, INT_MAX);
    dp[0] = 0;

    for (int c = 1; c <= C; c++) {
        for (int m : monedas) {
            if (c >= m && dp[c - m] != INT_MAX) {
                dp[c] = min(dp[c], dp[c - m] + 1);
            }
        }
    }

    cout << (dp[C] == INT_MAX ? -1 : dp[C]) << endl;

    return 0;
}
```

Entrada:

```text input
3 6
1 3 4
```

Salida:

```text output
2
```

Las denominaciones deben ser positivas y se pueden repetir sin límite. La respuesta es `3 + 3`; elegir siempre la mayor moneda daría tres monedas. El algoritmo cuesta $O(cantidadDeMonedas\cdot objetivo)$ tiempo y $O(objetivo)$ memoria. No sumes uno a `INT_MAX`: primero comprueba que el estado anterior sea alcanzable, como hace el código.

**Autocomprobación:** con monedas 2 y 4, el objetivo 3 debe devolver -1 y el objetivo 0 debe devolver 0. Estos resultados separan «imposible» de «solución vacía».

## Problema: Subsecuencia creciente más larga (LIS)

Dada una secuencia, encuentra la **subsecuencia creciente más larga**.

```
Secuencia: [10, 9, 2, 5, 3, 7, 101, 18]
LIS: [2, 3, 7, 18] → longitud 4
```

**Estado:** `dp[i]` = longitud de la LIS que termina en `i`.

**Transición:** `dp[i] = max(dp[j] + 1)` para todo `j < i` donde `v[j] < v[i]`.

```cpp
int lis(vector<int> &v) {
    if (v.empty()) return 0;
    int n = v.size();
    vector<int> dp(n, 1);

    for (int i = 1; i < n; i++) {
        for (int j = 0; j < i; j++) {
            if (v[j] < v[i]) {
                dp[i] = max(dp[i], dp[j] + 1);
            }
        }
    }

    return *max_element(dp.begin(), dp.end());
}
```

$O(n^2)$. Existe una versión $O(n \log n)$ con búsqueda binaria.

## Problema: Mochila 0/1

Tienes N objetos con peso y valor. Tu mochila aguanta un peso máximo W. ¿Cuál es el **valor máximo** que puedes cargar?

```cpp
int knapsack(vector<int> &peso, vector<int> &valor, int W) {
    int n = peso.size();
    vector<vector<int>> dp(n + 1, vector<int>(W + 1, 0));

    for (int i = 1; i <= n; i++) {
        for (int w = 0; w <= W; w++) {
            dp[i][w] = dp[i-1][w];  // No tomar objeto i
            if (w >= peso[i-1]) {
                dp[i][w] = max(dp[i][w], dp[i-1][w - peso[i-1]] + valor[i-1]);
            }
        }
    }

    return dp[n][W];
}
```

## Ejercicio de práctica

Sube una escalera de N escalones. En cada paso puedes subir 1 o 2 escalones. ¿De cuántas formas puedes llegar arriba?

<details>
<summary>Ver solución</summary>

```cpp
#include <iostream>
using namespace std;

int main() {
    int n;
    cin >> n;

    long long dp[105];
    dp[0] = 1;
    dp[1] = 1;

    for (int i = 2; i <= n; i++) {
        dp[i] = dp[i-1] + dp[i-2];
    }

    cout << dp[n] << endl;
    return 0;
}
```

Es exactamente Fibonacci: `dp[i] = dp[i-1] + dp[i-2]`.

</details>
