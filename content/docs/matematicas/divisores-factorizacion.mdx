...
**Antes de empezar:** trabajamos con enteros positivos. El 1 tiene un divisor positivo y ninguna factorización prima no vacía. Cero no entra en este modelo: todo entero positivo divide a cero, así que no tiene una lista finita de divisores positivos.

Un divisor elige independientemente el exponente de cada primo entre cero y el exponente presente en el número. Por eso, si $n=2^3\cdot3\cdot5$, la cantidad de divisores es $(3+1)(1+1)(1+1)=16$, no la suma de exponentes.
...
La pareja `(6,6)` aporta un solo divisor distinto. El recorrido cuesta $O(\sqrt n)$; ordenar los $d$ divisores añade $O(d\log d)$ y guardarlos ocupa $O(d)$. La condición por división evita desbordar el cuadrado del candidato.
...
**Autocomprobación:** prueba un primo, un cuadrado y 1. Los conteos son 2, un número impar de divisores, y 1, respectivamente. En SPF solo puedes factorizar valores dentro del límite preprocesado; no reserves una tabla hasta un número de 64 bits enorme.
