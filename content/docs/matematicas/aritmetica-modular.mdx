---
title: "Aritmética Modular"
description: "Domina las operaciones con módulo, esenciales para competencias de programación"
date: "2026-02-09"
author: "OOI Oaxaca"
tags: ["c++", "módulo", "aritmética modular", "exponenciación rápida"]
syllabusId: 43
---

## ¿Qué es el módulo?

**Antes de empezar:** necesitas residuo, GCD y potencias. El módulo debe ser positivo. Para una representación canónica de un entero posiblemente negativo, usa `(valor % modulo + modulo) % modulo` con tipos que soporten la suma intermedia.

La operación módulo (`%`) devuelve el **residuo** de la división. Es como un reloj: después de las 12, vuelve al 1.

- $17 \mod 5 = 2$ (porque $17 = 3 \times 5 + 2$)
- $10 \mod 3 = 1$
- $15 \mod 5 = 0$

**Analogía del reloj:** Si son las 10 y pasan 5 horas, son las 3 (no las 15). $15 \mod 12 = 3$.

## ¿Por qué es tan importante?

En competencias, las respuestas pueden ser **enormes** (cientos de dígitos). El problema dice: "imprime la respuesta módulo $10^9 + 7$". Esto significa que solo necesitas el residuo al dividir entre $10^9 + 7$.

```cpp
const int MOD = 1e9 + 7;  // 1000000007 (primo)
```

## Propiedades fundamentales

Las operaciones con módulo se pueden hacer **paso a paso**:

$$
(a + b) \mod m = ((a \mod m) + (b \mod m)) \mod m
$$

$$
(a \times b) \mod m = ((a \mod m) \times (b \mod m)) \mod m
$$

$$
(a - b) \mod m = ((a \mod m) - (b \mod m) + m) \mod m
$$

<Callout type="warning">
  La resta puede dar negativo en C++. Siempre suma `m` antes de tomar módulo:
  `((a - b) % m + m) % m`.
</Callout>

**¡La división NO funciona así!** No puedes hacer $(a / b) \mod m$. Necesitas el **inverso modular** (ver más abajo).

## Aplicando módulo en la práctica

```cpp
const int MOD = 1e9 + 7;

// Suma modular
long long sumar(long long a, long long b) {
    return (a + b) % MOD;
}

// Resta modular (cuidado con negativos)
long long restar(long long a, long long b) {
    return ((a - b) % MOD + MOD) % MOD;
}

// Multiplicación modular
long long multiplicar(long long a, long long b) {
    return (a % MOD) * (b % MOD) % MOD;
}
```

## Exponenciación rápida (modular)

Calcular $a^n \mod m$ directamente sería imposible para $n$ grande. Usamos la técnica de **exponenciación binaria**: $O(\log n)$.

**Idea:** $a^{10} = (a^5)^2 = (a^4 \times a)^2 = ((a^2)^2 \times a)^2$

```cpp
long long potencia(long long base, long long exp, long long mod) {
    long long resultado = 1 % mod;
    base = (base % mod + mod) % mod;

    while (exp > 0) {
        if (exp % 2 == 1) {           // Si el exponente es impar
            resultado = resultado * base % mod;
        }
        exp /= 2;                      // Dividir exponente entre 2
        base = base * base % mod;      // Elevar la base al cuadrado
    }

    return resultado;
}
```

### Ejemplo

$2^{10} \mod 1000$:

| Paso | base | exp | resultado              |
| ---- | ---- | --- | ---------------------- |
| 0    | 2    | 10  | 1                      |
| 1    | 4    | 5   | 1                      |
| 2    | 16   | 2   | 4 (×4, exp impar)      |
| 3    | 256  | 1   | 4                      |
| 4    | ...  | 0   | 1024 mod 1000 = **24** |

## Inverso modular

Para "dividir" en aritmética modular, multiplicamos por el **inverso modular**.

$a^{-1} \mod m$ es el número $x$ tal que $a \times x \equiv 1 \pmod{m}$.

Si $m$ es primo y $a$ no es divisible entre $m$, el Pequeño Teorema de Fermat da $a^{-1} \equiv a^{m-2}\pmod m$. En general, el inverso existe si y solo si $\gcd(a,m)=1$; con un módulo compuesto puedes calcularlo mediante Euclides extendido cuando esa condición se cumple.

```cpp
long long inverso(long long a, long long mod) {
    return potencia(a, mod - 2, mod);
}

// División modular: a / b mod m = a * b^(-1) mod m
long long dividir(long long a, long long b, long long mod) {
    return a % mod * inverso(b, mod) % mod;
}
```

Estas funciones de inverso por potencia suponen módulo primo y denominador no nulo módulo él. Por ejemplo, el inverso de 3 módulo 7 es 5 porque `3 * 5 % 7 = 1`; 6 módulo 9 no tiene inverso. No reemplaces la división entera de un enunciado por división modular sin justificarlo: calculan operaciones distintas.

## Ejemplo completo: Fibonacci grande

Calcula el N-ésimo Fibonacci módulo $10^9 + 7$:

```cpp
#include <iostream>
using namespace std;

const int MOD = 1e9 + 7;

int main() {
    int n;
    cin >> n;

    if (n <= 1) {
        cout << n << endl;
        return 0;
    }

    long long a = 0, b = 1;
    for (int i = 2; i <= n; i++) {
        long long c = (a + b) % MOD;
        a = b;
        b = c;
    }

    cout << b << endl;
    return 0;
}
```

## Ejercicio de práctica

Calcula $a^b \mod m$ dados a, b y m.

**Entrada:** `2 100 1000000007`
**Salida:** `976371285`

<details>
<summary>Ver solución</summary>

Suponemos exponente no negativo y `1 <= modulo <= 1000000007`; así, tras reducir factores, los productos caben en `long long`. Para módulos de 64 bits completos necesitas multiplicación modular más ancha.

```cpp test
#include <iostream>
using namespace std;

long long potencia(long long base, long long exp, long long mod) {
    long long res = 1 % mod;
    base = (base % mod + mod) % mod;
    while (exp > 0) {
        if (exp & 1) res = res * base % mod;
        base = base * base % mod;
        exp >>= 1;
    }
    return res;
}

int main() {
    long long a, b, m;
    cin >> a >> b >> m;
    cout << potencia(a, b, m) << endl;
    return 0;
}
```

Entrada de borde:

```text input
9 0 1
```

Salida:

```text output
0
```

Tomamos la convención de potencia con exponente cero igual a 1; su residuo módulo 1 es 0. La exponenciación usa $O(\log(exponente+1))$ tiempo y memoria adicional constante. Mantiene el producto de lo acumulado y la potencia pendiente equivalente a la potencia original.

</details>
