...
**Objetivo:** traducir el formato del enunciado a lecturas y escrituras exactas. `cin` lee la entrada estándar (**STDIN**) y `cout` escribe la salida estándar (**STDOUT**). En el juez esos flujos suelen estar conectados a archivos o tuberías, no a una persona: no imprimas preguntas como «Dame un número» salvo que el formato lo pida.
...
Si el juez espera `8`, agregar «El resultado es:» puede producir una respuesta incorrecta. Algunos jueces toleran ciertos espacios y otros comparan más estrictamente; no dependas de esa tolerancia. Respeta el formato especificado.

## Un número seguido de una línea completa

`cin >>` omite espacios iniciales y lee un token. `getline` lee hasta el siguiente salto de línea y sí conserva espacios dentro del texto. Si el nombre comienza en la **línea siguiente** a un número, elimina primero el resto de la línea del número:
...
`ignore()` sin argumentos descarta solo un carácter; podría no bastar si quedan espacios antes del salto. La versión mostrada supone que no necesitas otros datos de esa misma línea. `getline(cin >> ws, nombre)` es otra opción si puedes omitir espacios iniciales y líneas vacías; no la uses si esos espacios forman parte de los datos.

**Autocomprobación:** ¿qué pasaría si usaras `cin >> nombre`? Solo obtendrías `Ana`. Si no recibes entrada válida, `cin` activa un estado de error; un ciclo hasta EOF debe comprobar `while (cin >> valor)`, no asumir que siempre existe otro dato.
...
<Callout type="warning">
  Usa `'\n'` para saltos normales y `endl` o `flush` cuando necesites vaciar el
  búfer, como en una consulta interactiva. No redefinas `endl` con una macro:
  cambiarías su significado y podrías bloquear un protocolo que espera ese
  vaciado.
</Callout>
...
<Callout type="warning">
  Después de usar `ios_base::sync_with_stdio(false)`, **NO uses**
  `scanf`/`printf` (funciones de C) mezcladas con `cin`/`cout`. Elige uno u
  otro.
</Callout>
...
<Callout type="info">
  No necesitas memorizar `scanf`/`printf` si usas `cin`/`cout` con las
  optimizaciones. Son igualmente rápidos. Pero si los encuentras en soluciones
  de otros, ahora sabes qué significan.
</Callout>
...
[Jerarquía, operadores aritméticos, relacionales, lógicos y pre/post incremento](/blog/cpp-basico/operadores-aritmeticos).
