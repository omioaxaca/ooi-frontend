...
**Objetivo:** identificar qué se compila, dónde empieza la ejecución y qué aparece realmente en la salida. La configuración del compilador debe estar lista; todavía no necesitas funciones propias ni macros.
...
Ese encabezado es una extensión de la biblioteca de GCC, no parte de C++ estándar. Puede faltar con Clang y la biblioteca de Apple o con MSVC. En estas guías preferimos incluir los encabezados necesarios para que el código sea más fácil de entender y portar.
...
<Callout type="info">
  Los comentarios son tu mejor amigo. Cuando vuelvas a leer tu código después de
  una semana, los comentarios te recordarán qué estabas pensando. Úsalos
  generosamente.
</Callout>
...
**Sobre `endl` y `'\n'`:** ambos agregan un salto de línea, pero `endl` también vacía el búfer. Para salida normal suele bastar `'\n'`; en problemas interactivos sí puede ser necesario vaciar el búfer antes de esperar una respuesta.
...
La plantilla inicial necesita solamente el encabezado de salida y `main`. Añade herramientas conforme las necesites:
...
`int main()` debe seguir devolviendo `int`. No redefinas palabras del lenguaje con macros. Escribir `push_back`, tipos explícitos y nombres descriptivos permite relacionar el programa con su explicación.

**Autocomprobación:** cambia el texto por `42`. ¿El programa calcula algo o imprime esos dos caracteres? Imprime el contenido de la cadena. Si escribes `cout << 4 + 2`, calcula la suma y muestra 6. `return 0` indica éxito al sistema, pero no agrega un cero a la salida.
...
| `-Wall`       | Activar un conjunto amplio de advertencias | Puedes añadir `-Wextra` para más comprobaciones |
...
<Callout type="warning">
  Cuando el compilador te muestra un error, lee el mensaje cuidadosamente. Te
  dice el número de línea donde detectó el problema. A veces el error real está
  una o dos líneas antes de donde el compilador lo señala.
</Callout>
...
[Tipos de datos y variables](/blog/cpp-basico/tipos-de-datos).
