...
## Elegir el tipo antes de multiplicar
...
int main() {
  int cantidad = 100000;
  int precio = 100000;
  long long total = 1LL * cantidad * precio;
  double promedio = 7.0 / 2;
  cout << total << '\n' << promedio << '\n';
}
```
...
`1LL` hace que la multiplicación se evalúe como `long long`. Escribir solamente `long long total = cantidad * precio` seguiría multiplicando dos `int` antes de asignar. Para la división, convertir el resultado de `7 / 2` a `double` llega tarde: ya se truncó a 3.

**Autocomprobación:** un contador que empieza en cero debe inicializarse, por ejemplo `int contador = 0;`. Leer una variable local sin inicializar no equivale a leer cero. Prueba además si la **suma de muchos valores** cabe, aunque cada valor individual sí quepa en `int`.
...
Estos son los límites habituales de un `int` de 32 bits en los jueces, no un tamaño universal impuesto por C++. Puedes consultar `numeric_limits<int>::min()` y `max()` con `<limits>`. El mínimo exacto en 32 bits es -2147483648 y el máximo 2147483647.
...
<Callout type="warning">
  El desbordamiento de enteros **con signo** tiene comportamiento indefinido. No
  está garantizado que dé la vuelta ni que produzca un número concreto. Elige el
  tipo para los cálculos intermedios, no solamente para la variable donde
  guardarás el resultado.
</Callout>
...
cout << y << endl;     // No hay un resultado garantizado.
```
...
<Callout type="info">
  `double` suele ser una buena opción para cálculos decimales. La precisión de
  `long double` depende de la plataforma y en algunas coincide con `double`. El
  tipo correcto y la tolerancia dependen de las restricciones del problema.
</Callout>
...
<Callout type="info">
  Nota la diferencia: los `char` usan comillas **simples** (`'A'`), mientras que
  los `string` usan comillas **dobles** (`"Hola"`). Confundirlos es un error
  común.
</Callout>
...
<Callout type="warning">
  La **división entre enteros** siempre da un entero (trunca el decimal). `7 /
  2` da `3`, no `3.5`. Si necesitas el decimal, convierte al menos uno de los
  operandos a `double`.
</Callout>
...
[Entrada y salida de datos: STDIN y STDOUT](/blog/cpp-basico/entrada-salida).
